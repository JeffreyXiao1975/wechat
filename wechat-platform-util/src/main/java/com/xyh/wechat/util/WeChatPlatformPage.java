...
public class WeChatPlatformPage implements java.io.Serializable {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1925876673777446448L;
	//前一页 
	  ...
	  public WeChatPlatformPage() { 
	  } 
	  
	  /** 
	   * 常用,用于计算分页 
	   * */
	  public WeChatPlatformPage(Long totalRecords){    
	   ...
	   * 设置每页显示多少条时使用 
	   * */
	  public WeChatPlatformPage(Long everyPage,Long totalRecords){  
	  ...
	  private void setPageSortState(String newPageSortName){    
	    ...
	    if(StringUtils.isEmpty(sortName)){ 
	      //默认排序为升序 
	      this.sortState = WeChatPlatformPageUtil.ASC;    
	      this.sortInfo = WeChatPlatformPageUtil.PAGE_ASC;            
	    }else{ 
	      if(StringUtils.equalsIgnoreCase(newPageSortName, sortName)){ 
	        //判断sortState排序状态值 
	        if(StringUtils.equalsIgnoreCase(sortState, WeChatPlatformPageUtil.ASC)){ 
	          this.sortState = WeChatPlatformPageUtil.DESC; 
	          this.sortInfo = WeChatPlatformPageUtil.PAGE_DESC;                
	        }else{ 
	          this.sortState = WeChatPlatformPageUtil.ASC; 
	          this.sortInfo = WeChatPlatformPageUtil.PAGE_ASC;          
	        }         
